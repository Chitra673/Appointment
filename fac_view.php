<?php 
	...
	include "config.php";
	$row=mysqli_fetch_array(mysqli_query($conn,"select * from fac_reg where id='$uid'"));
	$path=$row['filepath'];
?>
<!DOCTYPE html>
<html>
	...
	<body>
		  ...
		  <div id="nav_div">
				...
				<table id="nav_table2";>
					...
					<tr class="nav_tr" id='trash' onmouseenter="menter(3)" onmouseout="mout(3)">
						<td class="nav_dt1"><form action='crlist.php' target='con_frame' method='post'><select name='yr'>
						<option value='E1' name='yr'>E1</option>
						<option value='E2' name='yr'>E2</option>
						<option value='E3' name='yr' checked>E3</option>
						<option value='E4' name='yr'>E4</option></select></td>
						<td class="nav_dt2"><input type='submit' name='crs' style='background-color:#90EE90' value='CR List'></form></td>
					</tr>
					<tr class="nav_tr" id="compose" onmouseenter="menter(4)" onmouseout="mout(4)">
						<td colspan=2><a href="compose.php" id='com' style="padding:4.5% 35% 4.5% 27%;font-size:20px;text-decoration:none;color:black" target="con_frame">Compose</a></td>
					</tr>
				</table>
		</div>
		<div id="con_div">
		...
		<script>
			function mout($x){
				...
				if($x==4)
					document.getElementById('compose').style.background="#ADD8E6";
			}
			function menter($x){
				if($x==1)
				...
				if($x==4)
					document.getElementById('compose').style.background="#65B9D4";
			}
			function id_back(){
