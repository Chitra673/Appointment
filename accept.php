<?php
	...
	$date=date('Y-m-d');
	$time=date('H:i:s');
	mysqli_query($conn,"UPDATE sentbox set accepted='a' WHERE sender='$sen' and receiver='$rec' and date='$d' and time='$t'");
	$rea="Request Accepted";
	if(mysqli_query($conn,"INSERT INTO sentbox values('$rec','$sen','$date','$time','$rea','a','$ad'"))
		echo "hello";
?>
<script>
	window.loction.href="inbox.php";
</script>
